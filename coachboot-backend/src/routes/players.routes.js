...
router.post('/', requireAuth, requireRole('head_coach', 'technical_director', 'fitness_coach'), [
  ...
  const { rows } = await pool.query(
    `INSERT INTO players (first_name,last_name,position,jersey_number,birthdate,team_id,status,form_score)
     VALUES ($1,$2,$3,$4,$5,$6,COALESCE($7,'Disponible'),COALESCE($8,70)) RETURNING *`,
    [first_name, last_name, position, jersey_number || null, birthdate || null, team_id || null, status, form_score]
  );
  res.status(201).json({ player: rows[0] });
}));

/** PUT /api/players/:id — mettre à jour un joueur */
router.put('/:id', requireAuth, requireRole('head_coach', 'technical_director', 'fitness_coach'), asyncHandler(async (req, res) => {
  ...
  const { rows } = await pool.query(
    `UPDATE players SET ${updates.join(', ')} WHERE id = $${params.length} RETURNING *`, params
  );
  if (!rows.length) return res.status(404).json({ error: 'Joueur introuvable.' });
  res.json({ player: rows[0] });
}));
...
router.delete('/:id', requireAuth, requireRole('head_coach', 'technical_director'), asyncHandler(async (req, res) => {
