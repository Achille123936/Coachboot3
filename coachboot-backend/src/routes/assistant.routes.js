...
async function buildClubContext() {
  const [{ rows: [playerStats] }, { rows: [nextMatch] }, { rows: [injuryStats] }] = await Promise.all([
    pool.query(`SELECT COUNT(*)::int AS total, COUNT(*) FILTER (WHERE status = 'Disponible')::int AS available FROM players`),
    pool.query(`SELECT home_team, away_team, match_date, competition FROM matches WHERE status = 'À venir' ORDER BY match_date ASC LIMIT 1`),
    pool.query(`SELECT COUNT(*)::int AS n FROM injuries WHERE status != 'Rétabli'`),
  ]);
